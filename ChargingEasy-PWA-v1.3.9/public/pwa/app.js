(() => {
  ...
  async function reverseGeocode(latitude, longitude) {
    ...
    if (cloud && cloud.reverseGeocode) {
      try {
        result = normalizeGeocodeResult(await cloud.reverseGeocode(latitude, longitude, language));
      } catch (error) {
  ...
  function wireChargeForm() {
    ...
    document.getElementById("use-location").addEventListener("click", () => {
      ...
      locationButton.innerHTML = '<span class="location-button-icon">' + icons.sync + '</span><span class="location-button-label">正在识别地址…</span>';
  ...
  function renderAbout() {
    ...
    page.innerHTML = [
      ...
      '<div class="about-legal"><a class="about-map-credit" href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">地址数据 © OpenStreetMap contributors</a><footer class="about-footer"><div class="about-signature-logo"><img src="./personal-logo.png" alt="LIU JUNJIE 个人标志"></div><p>All Rights Reserved © 2026 LIU JUNJIE</p></footer></div>',
      "</div>",
    ].join("");
  ...
  function dismissLaunchScreen() {
    ...
    const startedAt = Number(window.__chargingEasyLaunchStartedAt || performance.now());
    const delay = Math.max(0, 260 - (performance.now() - startedAt));
    window.setTimeout(() => {
      window.requestAnimationFrame(() => launchScreen.classList.add("is-hiding"));
      window.setTimeout(() => launchScreen.remove(), 380);
    }, delay);
  }
  ...
  async function init() {
    ...
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.register("./sw.js?v=1.3.9").catch((error) => console.error("Service worker:", error));
    }
