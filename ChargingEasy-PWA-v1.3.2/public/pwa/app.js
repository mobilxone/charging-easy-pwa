(() => {
  ...
  function openModal(content) {
    modalRoot.innerHTML = '<div class="modal-backdrop" role="presentation"><section class="modal-sheet" role="dialog" aria-modal="true">' + content + "</section></div>";
  ...
  function openAccountPopover(mode) {
    ...
    if (view === "otp") {
      content = [
        '<div class="account-popover-copy">', accountAvatar(""), '<h2>输入邮箱验证码</h2><p>验证码已发送至<br><strong>', escapeHTML(state.pendingEmail), "</strong></p></div>",
  ...
  function openRecordDetail(id) {
    ...
    if (!record) return;
    openModal([
      '<div class="modal-handle"></div><header class="modal-header"><h2>记录详情</h2><button class="modal-close" data-close-modal aria-label="关闭">×</button></header>',
