(() => {
  ...
  const icons = {
    ...
    sync: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 7v5h-5M4 17v-5h5"/><path d="M18.2 12A7 7 0 0 0 6.1 7.2L4 12M5.8 12a7 7 0 0 0 12.1 4.8L20 12"/></svg>',
  };
  ...
  function openModal(content, options) {
    ...
    modalRoot.innerHTML = '<div class="modal-backdrop' + (isRecordDetail ? " record-detail-layer" : "") + '" role="presentation"><section class="modal-sheet' + (isRecordDetail ? " record-detail-sheet" : "") + '" role="dialog" aria-modal="true">' + content + "</section></div>";
  ...
  function bindRecordDetailGestures() {
    ...
    function snapBack() {
      ...
      sheet.style.transition = "transform 240ms cubic-bezier(0.22, 0.78, 0.2, 1), opacity 180ms ease";
      sheet.style.transform = "translate3d(0, 0, 0)";
      sheet.style.opacity = "1";
      window.clearTimeout(resetTimer);
      resetTimer = window.setTimeout(clearInlineMotion, 260);
    }

    function finishDismiss(axis) {
      if (dismissing) return;
      dismissing = true;
      sheet.classList.remove("is-gesture-active");
      sheet.style.transition = "transform 190ms cubic-bezier(0.32, 0.72, 0, 1), opacity 160ms ease";
      sheet.style.transform = axis === "x"
        ? "translate3d(-58vw, 0, 0)"
        : "translate3d(0, 32vh, 0)";
      sheet.style.opacity = "0";
      layer.style.transition = "background-color 180ms ease";
    ...
    function onPointerDown(event) {
      ...
      const startedOnHandle = Boolean(event.target.closest(".modal-handle"));
      const startedOnControl = Boolean(event.target.closest("button, a, input, select, textarea, label"));
      if (!startedOnHandle && startedOnControl) return;

      gesture = {
        axis: startedOnHandle ? "y" : "x",
        id: event.pointerId,
    ...
    function onPointerMove(event) {
      ...
      if (gesture.axis === "y") {
        if (dy <= 0 || Math.abs(dx) > Math.abs(dy) * 1.15) return;
        gesture.distance = Math.min(dy, 280);
        gesture.active = gesture.distance > 4;
        suppressHandleClick = gesture.active;
        if (!gesture.active) return;
        event.preventDefault();
        sheet.classList.add("is-gesture-active");
        sheet.style.transform = "translate3d(0, " + gesture.distance + "px, 0)";
      } else {
        if (dx >= 0 || Math.abs(dy) > Math.abs(dx) * 0.85) return;
        gesture.distance = Math.min(-dx, 280);
        gesture.active = gesture.distance > 8;
        if (!gesture.active) return;
        event.preventDefault();
        sheet.classList.add("is-gesture-active");
        sheet.style.transform = "translate3d(-" + gesture.distance + "px, 0, 0)";
      }

      sheet.style.opacity = String(Math.max(0.68, 1 - gesture.distance / 700));
    }
    ...
    function onPointerEnd(event) {
      ...
      const shouldDismiss = gesture.active && (gesture.distance >= 78 || velocity >= 0.55);
      const axis = gesture.axis;
      gesture = null;
      try {
        if (sheet.hasPointerCapture(event.pointerId)) sheet.releasePointerCapture(event.pointerId);
      } catch (_error) {}
      if (shouldDismiss) finishDismiss(axis);
      else if (!dismissing) snapBack();
    }
  ...
  function accountPopoverShell(content) {
    return [
      ...
      '<button class="account-popover-close" type="button" data-close-modal aria-label="关闭">×</button>',
      content,
      "</section></div>",
    ].join("");
  }
  ...
  function openAccountPopover(mode) {
    ...
    if (view === "otp") {
      ...
      content = [
        ...
        '<div class="account-summary"><div><span>云端同步</span><strong>', state.autoSync ? "自动" : "手动", '</strong></div><div><span>同步时间</span><strong>', escapeHTML(syncTimeLabel()), "</strong></div></div>",
  ...
  function renderChargeForm() {
    ...
    page.innerHTML = [
      ...
      '<label class="form-row form-row-inline"><span class="form-label">具体地点</span><input class="inline-input" id="place" maxlength="80" placeholder="例如：信义区市府停车场" required></label>',
      '<button class="location-button" type="button" id="use-location"><span>', icons.location, "</span>使用当前位置</button></section>",
  ...
  function wireChargeForm() {
    ...
    document.getElementById("use-location").addEventListener("click", () => {
      ...
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        form.dataset.latitude = latitude;
        form.dataset.longitude = longitude;
        if (!document.getElementById("place").value) {
          document.getElementById("place").value = "坐标 " + latitude + ", " + longitude;
        }
        showToast("位置已获取，请补充城市名称");
      }, () => showToast("无法获取位置，请手动填写"), {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const record = normalizeRecord({
        ...
        place,
        location: place,
        latitude: form.dataset.latitude || null,
        longitude: form.dataset.longitude || null,
  ...
  function openRecordDetail(id) {
    ...
    if (!record) return;
    openModal([
      '<button class="modal-handle" type="button" aria-label="关闭记录详情"></button><header class="modal-header detail-modal-header"><h2>记录详情</h2></header>',
      ...
      '<div class="button-stack detail-actions"><button class="primary-button" id="edit-record">编辑记录</button><button class="danger-button" id="delete-record">删除这条记录</button></div>',
  ...
  function renderLineChart(points) {
    ...
    return [
      '<svg class="line-chart" viewBox="0 0 ', width, " ", height, '" role="img" aria-label="近六个月总费用趋势">',
      '<line class="grid" x1="0" y1="45" x2="320" y2="45"/><line class="grid" x1="0" y1="85" x2="320" y2="85"/>',
      '<path class="trend-line" d="', path, '"/>',
      coords.map((point) => '<circle class="point" cx="' + point.x + '" cy="' + point.y + '" r="4"/><text x="' + point.x + '" y="145" text-anchor="middle">' + point.label + "</text>").join(""),
      "</svg>",
    ].join("");
  }
  ...
  function renderStatistics() {
    ...
    page.innerHTML = [
      ...
      '<div><span>每公里费用</span><strong>', totals.costPerKm == null ? "—" : money(totals.costPerKm) + "/km", '</strong><small>全部费用 ÷ 行驶里程</small></div>',
      "</div></section>",
      '<section class="card"><h2>近6个月总费用</h2>', renderLineChart(recentSixMonths(state.statsReference)), "</section>",
      '<p class="settings-note">平均电价只计算充电费用；每公里费用包含充电、高速和停车三类支出。电量与能耗数据仅包含充电记录。</p>',
      "</div>",
    ].join("");
