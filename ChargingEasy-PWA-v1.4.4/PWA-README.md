...
将项目上传到 GitHub 后，在 Cloudflare Pages 中使用以下配置：
...
`public/_redirects` 会把网站根地址自动跳转至 `/pwa/`。
...
充电记录以当前设备的 IndexedDB 为第一数据源；登录账户后，可通过 Supabase 自动或手动同步到云端。离线时仍能继续记录，恢复网络后再补传。建议继续定期在设置中导出 JSON 备份。
