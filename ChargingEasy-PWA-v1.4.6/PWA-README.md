...
- 日期可直接手动输入并自动识别，记录只保存年月日，不保存充电时间
- 记录搜索、类型筛选、详情、编辑与删除
- 月度与年度统计、费用分类和行驶里程趋势
- 最近 6 个月趋势支持按单个月份连续拖动，年度趋势按单个年份拖动
...
将解压后的 `ChargingEasy-PWA-v1.4.6` 文件夹内容上传到 GitHub 仓库，然后在 Cloudflare Pages 使用：
...
如果 GitHub 仓库多嵌套了一层 `ChargingEasy-PWA-v1.4.6`，请把 Cloudflare 的 Root directory 设置为该文件夹名。
