(() => {
  ...
  function canonicalDateFromParts(year, month, day) {
    if (!validDateParts(year, month, day)) return null;
    return String(year).padStart(4, "0") + "-" + String(month).padStart(2, "0") + "-" + String(day).padStart(2, "0");
  }

  function datePartsFromStored(value) {
    if (typeof value === "string" && /^\d{4}-\d{2}-\d{2}$/.test(value)) {
      const parts = value.split("-").map(Number);
      return validDateParts(parts[0], parts[1], parts[2]) ? parts : null;
    }
  ...
  function canonicalRecordDate(value) {
    const parts = datePartsFromStored(value);
    if (!parts) throw new RangeError("Invalid record date");
    return canonicalDateFromParts(parts[0], parts[1], parts[2]);
  }

  function recordDate(value) {
    const parts = datePartsFromStored(value);
    return parts ? new Date(parts[0], parts[1] - 1, parts[2], 12) : new Date(NaN);
  }

  function compareRecordDates(first, second) {
    const dateDifference = recordDate(first.date) - recordDate(second.date);
  ...
  function normalizeRecord(record) {
    ...
    const common = {
      ...
      kind,
      date: canonicalRecordDate(record.date),
  ...
  async function getAllRecords() {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        db.close();
        try {
          resolve(request.result.map(normalizeRecord).sort((a, b) => compareRecordDates(b, a)));
        } catch (error) {
          reject(error);
        }
      };
  ...
  function sameYear(date, reference) {
    return recordDate(date).getFullYear() === (reference || new Date()).getFullYear();
  }
  ...
  function longDate(date) {
    return new Intl.DateTimeFormat("zh-CN", {
      ...
      month: "long",
      day: "numeric",
    }).format(recordDate(date));
  }

  function shortDate(date) {
    return new Intl.DateTimeFormat("zh-CN", { month: "numeric", day: "numeric" }).format(recordDate(date));
  }

  function parseManualDate(input, reference) {
    const today = reference instanceof Date && !Number.isNaN(reference.getTime()) ? reference : new Date();
    let value = String(input || "").trim().replace(/[０-９]/g, (digit) => String(digit.charCodeAt(0) - 65296));
    if (!value) return null;
    if (value === "今天" || value === "今日") return canonicalRecordDate(today);
    if (value === "昨天" || value === "昨日") {
      const yesterday = new Date(today.getFullYear(), today.getMonth(), today.getDate() - 1, 12);
      return canonicalRecordDate(yesterday);
    }
    if (value === "前天") {
      const dayBeforeYesterday = new Date(today.getFullYear(), today.getMonth(), today.getDate() - 2, 12);
      return canonicalRecordDate(dayBeforeYesterday);
    }
    ...
    return canonicalDateFromParts(year, month, day);
  }

  function formatDateInput(value) {
    const canonical = canonicalRecordDate(value || new Date());
    return canonical.replaceAll("-", "/");
  }

  function readManualDate(input) {
    const canonical = parseManualDate(input.value);
    input.classList.toggle("is-invalid", !canonical);
    input.setAttribute("aria-invalid", canonical ? "false" : "true");
    if (canonical) input.value = formatDateInput(canonical);
    return canonical;
  }

  function wireManualDateInput(input, onRecognized) {
    input.addEventListener("input", () => {
      input.classList.remove("is-invalid");
      input.setAttribute("aria-invalid", "false");
    });
    input.addEventListener("blur", () => {
      const canonical = readManualDate(input);
      if (canonical && onRecognized) onRecognized(recordDate(canonical));
    });
  }
  ...
  function renderRecords() {
    ...
    filtered.forEach((record) => {
      const date = recordDate(record.date);
  ...
  function renderChargeForm() {
    ...
    page.innerHTML = [
      ...
      '<label class="form-row form-row-inline date-form-row"><span class="form-label">充电日期</span><span class="date-entry"><input class="inline-input date-input" id="date" type="text" inputmode="numeric" autocomplete="off" autocapitalize="off" spellcheck="false" maxlength="12" value="', formatDateInput(), '" placeholder="YYYY/MM/DD" aria-describedby="charge-date-hint" required><small id="charge-date-hint">支持 20260824 或 0824</small></span></label>',
      "</section>",
  ...
  function wireChargeForm() {
    ...
    const calculate = () => {
      ...
      const canonical = parseManualDate(dateInput.value);
      const date = canonical ? recordDate(canonical) : new Date(NaN);
      const previous = Number.isNaN(date.getTime()) ? null : previousChargeBefore(date);
    ...
    wireManualDateInput(dateInput, calculate);
  ...
  function renderExpenseForm(kind) {
    ...
    page.innerHTML = [
      ...
      '<label class="form-row form-row-inline date-form-row"><span class="form-label">日期</span><span class="date-entry"><input class="inline-input date-input" id="date" type="text" inputmode="numeric" autocomplete="off" autocapitalize="off" spellcheck="false" maxlength="12" value="', formatDateInput(), '" placeholder="YYYY/MM/DD" aria-describedby="expense-date-hint" required><small id="expense-date-hint">支持 20260824 或 0824</small></span></label>',
      fields,
    ...
    wireManualDateInput(expenseDateInput);
    document.getElementById("expense-form").addEventListener("submit", async (event) => {
      event.preventDefault();
      const cost = Number(document.getElementById("cost").value);
      const canonical = readManualDate(expenseDateInput);
      if (cost < 0 || !canonical) {
  ...
  function openEditRecord(record) {
    openModal([
      ...
      '<label class="form-row edit-date-row"><span class="form-label">日期</span><input class="field date-input" id="edit-date" type="text" inputmode="numeric" autocomplete="off" autocapitalize="off" spellcheck="false" maxlength="12" value="', formatDateInput(record.date), '" placeholder="YYYY/MM/DD" aria-describedby="edit-date-hint" required><small class="date-field-hint" id="edit-date-hint">支持 20260824、8月24日或 0824</small></label>',
    ...
    wireManualDateInput(editDateInput);
    document.getElementById("edit-form").addEventListener("submit", async (event) => {
      event.preventDefault();
      const canonical = readManualDate(editDateInput);
  ...
  function chartData(reference, period) {
    ...
    periodRecords(reference, period).forEach((record) => {
      const index = period === "month"
        ? Math.min(4, Math.floor((recordDate(record.date).getDate() - 1) / 7))
        : recordDate(record.date).getMonth();
  ...
  function monthlyTrendSeries(reference, metric) {
    ...
    const eligibleDates = state.records
      ...
      .map((record) => recordDate(record.date))
    ...
    const earliest = eligibleDates.length
      ...
      : null;
    const minimumStart = new Date(end.getFullYear(), end.getMonth() - 11, 1);
    const start = earliest && earliest < minimumStart
  ...
  function yearlyTrendSeries(reference, metric) {
    const endYear = reference.getFullYear();
    const eligibleYears = state.records
      .filter((record) => recordContributesToTrend(record, metric))
      .map((record) => recordDate(record.date))
      .filter((date) => !Number.isNaN(date.getTime()) && date.getFullYear() <= endYear)
      .map((date) => date.getFullYear());
    const earliestYear = eligibleYears.length ? Math.min(...eligibleYears) : endYear - 2;
    const startYear = Math.min(endYear - 2, earliestYear);
  ...
  function renderLineChart(points, ariaLabel, options) {
    ...
    const baseWidth = 320;
    const height = 150;
    const visibleCount = settings.visibleCount || 6;
    const left = settings.showValues ? 22 : 12;
    const right = settings.showValues ? 22 : 12;
    const top = settings.showValues ? 32 : 14;
    const bottom = 24;
    const step = (baseWidth - left - right) / Math.max(1, visibleCount - 1);
    const width = left + right + step * Math.max(0, points.length - 1);
    const cssWidth = Math.max(100, width / baseWidth * 100);
    ...
    return [
      '<div class="trend-scroller" data-visible-count="', visibleCount, '" data-step-ratio="', step / width, '" data-trend-unit="', settings.unit || "cost", '" data-trend-payload="', payload, '">',
      '<svg class="line-chart" style="width:', cssWidth, '%" viewBox="0 0 ', width, " ", height, '" role="img" aria-label="', escapeHTML(ariaLabel), '">',
      '<line class="grid" x1="0" y1="45" x2="', width, '" y2="45"/><line class="grid" x1="0" y1="85" x2="', width, '" y2="85"/>',
  ...
  function initializeTrendScrollers() {
    document.querySelectorAll(".trend-scroller").forEach((scroller) => {
      let payload = [];
      try {
        payload = JSON.parse(decodeURIComponent(scroller.dataset.trendPayload || ""));
      } catch {
        return;
      }
      const visibleCount = Math.max(1, Number(scroller.dataset.visibleCount || 1));
      const stepRatio = Number(scroller.dataset.stepRatio || 0);
      const card = scroller.closest(".trend-card");
      const rangeLabel = card && card.querySelector("[data-trend-range]");
      const totalLabel = card && card.querySelector("[data-trend-total]");
      let snapTimer = 0;

      function actualStep() {
        return scroller.scrollWidth * stepRatio;
      }

      function visibleStartIndex() {
        const stepValue = actualStep();
        const maxStart = Math.max(0, payload.length - visibleCount);
        return stepValue > 0 ? Math.max(0, Math.min(maxStart, Math.round(scroller.scrollLeft / stepValue))) : 0;
      }

      function updateWindowLabels() {
        const startIndex = visibleStartIndex();
        const visible = payload.slice(startIndex, startIndex + visibleCount);
        if (!visible.length) return;
        if (rangeLabel) rangeLabel.textContent = visible[0].fullLabel + " – " + visible[visible.length - 1].fullLabel;
        if (totalLabel) {
          const total = visible.reduce((sum, point) => sum + Number(point.value || 0), 0);
          totalLabel.textContent = scroller.dataset.trendUnit === "distance" ? "合计 " + number(total, 0) + " km" : "合计 " + money(total);
        }
      }
      ...
      function snapToNearestPoint() {
        const stepValue = actualStep();
        if (!stepValue) return;
        const target = visibleStartIndex() * stepValue;
        if (Math.abs(scroller.scrollLeft - target) < 1) return;
        scroller.scrollTo({ left: target, behavior: reducedMotionQuery.matches ? "auto" : "smooth" });
      }

      scroller.addEventListener("scroll", () => {
        updateWindowLabels();
        clearTimeout(snapTimer);
        snapTimer = setTimeout(snapToNearestPoint, 130);
      }, { passive: true });
  ...
  function renderTrendCard(reference) {
    ...
    const points = annual ? yearlyTrendSeries(reference, metric) : monthlyTrendSeries(reference, metric);
    const visibleCount = annual ? 3 : 6;
    const canScroll = points.length > visibleCount;
    const metrics = [
    ...
    return [
      '<section class="card trend-card"><div class="trend-heading"><div><h2>', annual ? "近3年费用" : "近6个月费用", '</h2><span>', canScroll ? (annual ? "向右拖动查看更早年份" : "向右拖动查看更早月份") : (annual ? "暂无更早年份数据" : "最近六个月"), '</span></div></div>',
      ...
      '</div><div class="trend-window-meta"><span data-trend-range></span><strong data-trend-total></strong></div>',
      renderLineChart(points, (annual ? "年度" : "月度") + trendMetricLabel(metric) + "连续趋势", { visibleCount }),
      "</section>",
    ].join("");
  }
  ...
  function renderDistanceTrendCard(reference) {
    const annual = state.statsPeriod === "year";
    const points = annual ? yearlyTrendSeries(reference, "distance") : monthlyTrendSeries(reference, "distance");
    const visibleCount = annual ? 3 : 6;
    const canScroll = points.length > visibleCount;
    const title = annual ? "近3年行驶里程" : "近6个月行驶里程";
    const hint = canScroll
      ? (annual ? "向右拖动查看更早年份" : "向右拖动查看更早月份")
    ...
    return [
      ...
      '<div class="trend-window-meta"><span data-trend-range></span><strong data-trend-total></strong></div>',
      renderLineChart(points, (annual ? "年度" : "月度") + "行驶里程连续趋势", {
        visibleCount,
        unit: "distance",
        showValues: true,
  ...
  async function init() {
    ...
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.register("./sw.js?v=1.4.6").catch((error) => console.error("Service worker:", error));
    }
